...
export class Drake2Command extends Command {
  public constructor(context: Command.LoaderContext, options: Command.Options) {
    super(context, {
      ...options,
      name: 'drake2',
      description: 'Generates a drake meme with Danny Fenton',
  ...
  public async messageRun(message: Message) {
    ...
    if (parts.length < 2 || !parts[0] || !parts[1]) {
      return message.reply('Please provide both top and bottom text separated by `|`. Usage: `drake2 <top_text> | <bottom_text>`');
    }
  ...
  private async generateMeme(message: Message, top: string, bottom: string) {
    ...
    const response = await axios.post(
      'https://apis.duncte123.me/memes/dannyphantomdrake',
      formData,
      {
